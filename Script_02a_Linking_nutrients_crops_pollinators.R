##################################################################################################################################################################
##############   MICRO-POLL SCRIPT 2a - LINKING CROPS, POLLINATORS & MICRONUTRIENTS TO CALCULATE & PLOT CONTRIBUTIONS  ############################################
##################################################################################################################################################################
...
pollen_proportion_summary <- pollen_transport_summary %>%  dplyr::group_by(plant_sci_name) %>%  
  dplyr::mutate(proportion_pollen = total_pollen_trans / sum(total_pollen_trans))
...
poll_crop_nutrient_data_calcs <- poll_crop_nutrient_data %>%  
  dplyr::mutate(poll_contribution = prop_contribution * proportion_pollen * final_poll_dependence)
...
poll_contribution_wide <- poll_contribution_wide %>%  
  dplyr::mutate(mean_contribution = (Calcium + FolateTotal + Iron + VitARE + VitaminC + VitaminE)/6)
...
poll_contribution_plant_indirect <- poll_contribution_plant_indirect %>%  
  dplyr::mutate(plant_IC_score = poll_contribution * prop_plant_use)
